use crate::utils::{helpers, probability};
...
pub fn problem_9() {
    let start = Instant::now();
    let mut answer = 0;
    let combinations = probability::generate_combinations(3, 997);
...
pub fn problem_10() {
    ...
    let sum: u64 = primes.into_iter().sum();
...
pub fn problem_12() {
    let start = Instant::now();
    const START_AT: i32 = 44;

    let mut num_triagle = START_AT;
    loop {
        let triangle_number = helpers::triangle_number(num_triagle);
        let divisors = helpers::num_divisors(triangle_number);
    ...
    println!(
        " {} answer: {} duration: {:.2?}",
        function_name!(),
        helpers::triangle_number(num_triagle),
        duration
    );
}
...
pub fn problem_13() {
    ...
    for number in numbers.iter() {
        let bi = helpers::string_to_biguint(number);
        sum += bi;
    }

    let sum_str = sum.to_string();
    let answer: String = sum_str.chars().take(10).collect();
...
// (Collatz Problem) Which starting number, under one million, produces the longest chain?
#[allow(dead_code)]
#[named]
pub fn problem_14() {
    let start = Instant::now();
    ...
    for n in 1..1_000_000 {
        let x: u64 = n;
        let count = helpers::collatz_length(x);
        if count > longest {
            longest = count;
            starting_value = n;
        }
...
pub fn problem_15() {
    ...
    // there are 20 lefts and 20 rights. Therefore there are 40 choices
    // 40 * 39 * 38 ... 1
    // but L1 * L2 is the same as L2 * L1 so we have to remove duplicates
    // theredfore we want all the combinations

    // answer = n! / (k! * (n-k)!)
    // or answer = 40! / 20! * 20!

    let fourty:BigUint = BigUint::from(40u32);
    let twenty:BigUint = BigUint::from(20u32);

    let mut answer = probability::factoria_biguint(&fourty) / probability::factoria_biguint(&twenty);
    answer = answer / probability::factoria_biguint(&twenty);
...
// Picking a path where the number below adjacent to the one above
#[allow(dead_code)]
#[named]
pub fn problem_18() {
    let start = Instant::now();
    ...
    // work from the bottom up, find the "adjacent" nodes that lead to a local maximum
